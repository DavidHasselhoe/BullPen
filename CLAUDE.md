...
| `/api/cron/instagram-earnings-publish` | `0 11 * * 1` | Publish whatever `instagram-earnings-weekly` staged the day before (looked up by the same ISO-week `period_key`), if it's still `status: 'ready'`. No human approval step in between — the Sunday Discord preview is the review window. See `scripts/publish-instagram.ts` for the manual/on-demand equivalent and `docs/instagram-setup.md` for Meta credential setup. |
