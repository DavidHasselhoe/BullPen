...
Optional but used in production: `FINNHUB_API_KEY`, `RESEND_API_KEY`, `CRON_SECRET`, `LOGO_DEV_KEY`, `NEXT_PUBLIC_APP_URL`, `ANTHROPIC_API_KEY` (Why Today? + Daily Brief), `SNAPTRADE_CLIENT_ID`, `SNAPTRADE_CONSUMER_KEY`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `DISCORD_CHANGELOG_WEBHOOK_URL` (Discord changelog announcements — see `scripts/post-changelog-discord.ts`), `DISCORD_INSTAGRAM_WEBHOOK_URL`, `INSTAGRAM_APP_ID`, `INSTAGRAM_APP_SECRET`, `INSTAGRAM_ACCESS_TOKEN`, `INSTAGRAM_USER_ID` (automated Instagram content pipeline, Business Login for Instagram — see `docs/instagram-setup.md`), `DISCORD_INDEX_SYNC_WEBHOOK_URL` (index-constituent sync change notifications — see `scripts/sync-index-constituents.ts`), `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST` (product analytics, gated behind cookie consent — see `components/analytics/PostHogProvider.tsx`), `BULLPEN_POSTAL_ADDRESS` (CAN-SPAM footer address; `lib/email/footer.ts` refuses a marketing send without it), `DMCA_AGENT_NAME` / `DMCA_AGENT_ADDRESS` / `DMCA_AGENT_PHONE` / `DMCA_AGENT_EMAIL` / `DMCA_AGENT_REGISTERED_ON` (published on `/dmca` once the agent is registered with the U.S. Copyright Office — see `lib/legal/dmca-agent.ts`).

### Legal compliance surfaces

Four things are load-bearing and easy to break without noticing:

- **Age gate.** Signup collects a date of birth and refuses under-13s (COPPA). The rule lives in `lib/auth/age-gate.ts` *and* in migration `146_age_gate.sql` (a trigger, because signup runs client-side and form validation is not a boundary). `AgeCheckGate` in the root layout asks accounts that arrived without one, which is every Google sign-in and everything created before 2026-09-22. Whether 13-17 should be allowed at all is an open decision, noted in `AGE_POLICY_NOTE`.
- **Email footers.** Every send goes through `sendEmail`, which requires a `kind` of `transactional` or `marketing` and appends the matching footer. Marketing mail without `BULLPEN_POSTAL_ADDRESS` throws rather than sends.
