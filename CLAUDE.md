...
| `/api/cron/instagram-earnings-weekly` | `0 12 * * 0` | Generate + stage next week's earnings-calendar Instagram carousel, notify Discord for review. |
| `/api/cron/instagram-earnings-publish` | `0 11 * * 1` | Publish whatever `instagram-earnings-weekly` staged the day before (looked up by the same ISO-week `period_key`), if it's still `status: 'ready'`. No human approval step in between — the Sunday Discord preview (with a one-click "Publish Now" button, see below) is the review window. See `scripts/publish-instagram.ts` for the manual/on-demand equivalent and `docs/instagram-setup.md` for Meta credential setup. |
| `/api/cron/instagram-earnings-results` | `0 14 * * 6` | Generate + stage the past week's beat/missed earnings-results recap carousel, notify Discord for review. Own `content_type` (`earnings_results`), can share a `period_key` with an `earnings_calendar` row for the same week without colliding. |
| `/api/cron/instagram-earnings-results-publish` | `0 15 * * 0` | Publish whatever `instagram-earnings-results` staged the day before, if still `status: 'ready'` — mirror image of `instagram-earnings-publish` for this content type. |
