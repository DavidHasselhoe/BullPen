...
| `/api/cron/refresh-congress-trades` | `0 14 * * 2,5` | Washington Trading: `--refresh` sweep (20 newest trades per tracked member, ~665 Disclosed Capitol credits a run at 19 members) and notify followers (`user_politician_follows`) of newly stored trades. Twice weekly because PTRs are filed 30-46 days after the trade. Does not refresh positions (per-row, unbounded cost: `npm run ingest-congress -- --holdings-only --dry-run` first). Needs `DISCLOSED_CAPITOL_API_KEY` in Vercel |
