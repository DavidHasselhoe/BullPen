...
/**
 * "Use my holdings as the foundation" — the consent and the preview in one
 * control.
 *
 * Nothing about someone's portfolio should reach a model because of a setting
 * they turned on months ago, so this is per build and off by default, and the
 * positions it would send are listed right here before anything is submitted.
 * The weights shown are the same cost-basis weights the prompt sends, so the
 * screen and the request cannot disagree.
 *
 * Hidden entirely for an investor with no holdings: a toggle that silently
 * does nothing is worse than no toggle.
 */
export function HoldingsFoundationToggle({
  enabled,
  onChange,
  disabled,
}: {
    ...
    <div className="rounded-xl border border-border/60 bg-card/60">
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-foreground">{t('portfolioBuilderUseHoldingsTitle')}</p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t('portfolioBuilderUseHoldingsDescription')}
          </p>
        </div>
        <Switch
          checked={enabled}
          ...
          disabled={disabled}
          aria-label={t('portfolioBuilderUseHoldingsTitle')}
        />
      </div>
