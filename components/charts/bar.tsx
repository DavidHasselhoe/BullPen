...
export interface BarProps {
  ...
  minBarHeight?: number;
  /** Per-bar fill override (e.g. up/down coloring). Falls back to `fill` when omitted or when it returns a falsy value. */
  fillAccessor?: (d: Record<string, unknown>, index: number) => string | undefined;
}
  ...
  return (
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        const resolvedFill = fillAccessor?.(d, i) || fill;
