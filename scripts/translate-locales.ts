...
- That list is exhaustive: it does NOT include ordinary words, adjectives, language/country/nationality names, or BullPen's own in-app feature names (e.g. "Deep Dive", "Portfolio Builder", "Market Mood", "Dividend Calculator", "Stock Screener" — these are plain descriptive phrases, not registered trademarks, and every other financial app names its tools in its own local language). Every value you receive that isn't one of those exact DO-NOT-TRANSLATE terms must be translated into ${name}, including short capitalized phrases and single words like "Live", "English", "Spanish" — capitalization alone never means "leave this in English."
 ...
 * interpolation placeholders, whitespace, punctuation, math/currency symbols,
 * and plain digits — i.e. there is no actual translatable content left once
 * those are stripped out. A string like "{{ticker}} EPS" legitimately
 * round-trips unchanged in most target languages (nothing in it can be
 * translated), but its literal form isn't itself a DO_NOT_TRANSLATE entry,
 * so the plain unchanged-string check below would otherwise reject it.
 * Reproduced live 2026-08-26 on `compareTickerEpsHeader`. Digits are
 * stripped too: a string like "{{count}} / 500" is pure numerals and
 * punctuation with nothing to translate, and failed validation in all 6
 * languages until this was added. Symbols (Unicode category `Sm`/`So`, e.g.
 * "×") are a separate Unicode category from punctuation (`P`) and were still
 * failing on a string like "(×)" until included here too.
 */
function isEntirelyDoNotTranslate(text: string): boolean {
  ...
  remainder = remainder.replace(/[\s\p{P}\p{S}\d]+/gu, '');
