...
import { tierFromInt } from '@/lib/billing/tier';
...
export default function AdminCostsPage() {
  ...
  const tier = tierFromInt(user?.account_tier ?? null);
  ...
  if (authLoading) {
    return <main className="container mx-auto max-w-6xl py-8 px-4">Loading…</main>;
  }
    ...
    return (
      <main className="container mx-auto max-w-6xl py-8 px-4">
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-6 flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold">Admin only</h3>
            <p className="text-sm text-muted-foreground mt-1">
              This page is restricted to admin accounts (account_tier = 2).
            </p>
          </div>
        </div>
      </main>
    ...
    <main className="container mx-auto max-w-6xl py-8 px-4 sm:px-6 lg:px-8">
      <header className="mb-8">
        ...
        <p className="text-sm text-muted-foreground/80 mt-1">
          Last 7 days of `ai_usage` aggregated by feature and user.
        </p>
      </header>
