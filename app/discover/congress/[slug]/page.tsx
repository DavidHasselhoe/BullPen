...
export async function generateMetadata({
  ...
  const description = position
    ? `Stock trades disclosed by ${displayName} (${position}) under the STOCK Act, with filing dates and reported amount ranges.`
    : `Stock trades disclosed by ${displayName} under the STOCK Act, with filing dates and reported amount ranges.`;

  return {
    title,
    description,
    alternates: { canonical: `${BASE_URL}/discover/congress/${slug}` },
    openGraph: { title, description, url: `${BASE_URL}/discover/congress/${slug}` },
  };
}
