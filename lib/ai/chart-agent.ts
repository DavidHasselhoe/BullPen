...
import { COMPANY_DATA_TOOLS, navigateTo } from './tools';

const ALL_TOOLS = { ...CHART_TOOLS, ...COMPANY_DATA_TOOLS, navigateTo };
...
You can also send the user to another page in the app with navigateTo: the watchlist, price alerts, Academy, the Portfolio Builder, the market calendar and the rest (its tool description lists every destination). Use destination "stock" to leave this page for another ticker’s own stock page, rather than switching the current chart, when that is what the user wants; "stock_fullscreen" for that ticker’s fullscreen chart view.
